...
**Decision**: LOOKING transitions to INTERACTING the moment `mc.hitResult instanceof BlockHitResult` AND `((BlockHitResult) mc.hitResult).getBlockPos().equals(target)` — the client's raycast points at the target block. No angular-convergence requirement; the camera keeps easing toward its aim point during INTERACTING. (The pre-attack hesitation still sits between the gate firing and the first attack.)

### Alternatives
| Approach | Pros | Cons |
|----------|------|------|
| Angular tolerance only (original) | Simple | When an obstacle sits in the line of sight, the camera can be aimed within tolerance of the target vector while the raycast lands on the obstacle. The bot then transitions to INTERACTING and visibly starts attacking the wrong block. Reported as "wird angefangen abzubauen bevor der richtige Block anvisiert wird". |
| Angular + hit-result (previous) | Hit-result is authoritative; angular check throttles the polling | The angular gate delays the attack past the moment a human would click: the crosshair is already on the block (hit-result true) but the bot keeps staring until the camera is within 5° of the *ideal* aim point. Reported as "abbauen soll anfangen sobald der spieler den block anvisiert und nicht erst nach der kamera ausrichtung". The throttle argument is void — `mc.hitResult` is a field read. |
| Hit-result only (chosen) | Fires exactly when a human would click: crosshair touches the block. The obstacle correctness lives entirely in the hit-result check (raycast on obstacle → gate holds → `lookTimeout` fails cleanly), proven by the mine-blocked-by-obstacle test. Camera convergence continues during INTERACTING, which already re-aims every tick. | The carried-item packet window shrinks when the crosshair is already near the target at LOOKING entry (gate can fire within 1–2 ticks); covered by the per-tick resend in LOOKING, the defensive resend on INTERACTING tick 1, and the fact that the server only computes drops at break *completion*, several ticks later. |
| Server-side reach check | Authoritative | Rejection is silent and arrives after the visible mis-aim; doesn't address either reported symptom. |

Chose hit-result only because both reported symptoms bracket the same truth: the crosshair-on-block raycast — not camera convergence — is when mining should begin. The angular gate added latency without adding correctness once the hit-result check existed.
