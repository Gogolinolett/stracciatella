...
**The thresholds are targets, not calibrations, and the miner does not meet them.** An earlier revision set them from what the miner happened to do, which made the suite certify the slowness it was written to expose — a bar drawn round the implementation can only ever agree with it. They now come from the game's constants and the intended design: stone is hardness 1.5 against a diamond pickaxe's speed 8, so `8/1.5/30` per tick is **6 ticks of breaking per block**, and with the step and the aim overlapping the break as designed, 12 ticks per block is twice the floor. Likewise 90% duty ("always mining"), 45 degrees of yaw per block (one aim per column, one turn per row), a 35-tick gap ceiling (the longest pause the design sanctions: a 25-tick breather plus a 10-tick reaction), and 90% of drops collected ("very rare break").

Current state against those: 25 ticks and 71-92 degrees per block, 67-70% duty, gaps of 16-24 (74 on the aim patch), 16-17 of 18 drops. All three fail on the budget gate, which fires before the quality gates and so masks them until the speed moves. One number from the failing runs is worth keeping in view: the resume run reaches **100% duty and still misses the budget**, so the cost is not only in the pauses between blocks.
